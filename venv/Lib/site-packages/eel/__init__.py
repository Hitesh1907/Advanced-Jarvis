        ...
        if call_id in _call_return_callbacks:
            ...
            if message['status'] == 'ok':
                if 'value' in message:
                    ...
                    print(f"Warning: 'value' missing in success message: {message}")
                    callback(None)
            elif message['status'] == 'error' and error_callback is not None:
                error_callback(message.get('error'), message.get('stack'))
        else:
            if 'value' in message:
                _call_return_values[call_id] = message['value']
            else:
                print(f"Warning: 'value' missing in return message: {message}")
                _call_return_values[call_id] = None
